...
As you can see in the code snippet, above I opted to scale the features. Researching that topic, I found that there is a multitude of approaches, where some use a combination of `StandardScaler`, `MinMaxScaler` and `Normalizer`. Scale only the continuous features and not the categorical, etcetera. For now, I just used the most straightforward way.
