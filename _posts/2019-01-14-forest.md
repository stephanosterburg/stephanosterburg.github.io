...
tags: [data, code, forest]
image:
  path: /images/dataset-cover.jpeg
  feature: dataset-cover.jpeg
---

## Or who cannot see the wood for the trees?
...
Because of the sheer number of soil types and wilderness areas I had a look at them first. Luckily, the researchers here were efficient in the sense that they document very thing meticulous. Everything we need to know with a detailed description can be found online[^3]. 
...
Not that this is from any importance for our data exploration or to make a prediction, but I found it interesting to read. Because it makes you think about what kind of implications can your result potentially have.

However, what we want to find out is how is the data distribute by `Cover_Type`. Here I created a `seaborn.countplot`.
...
Now, unlike categorical data, the continues data is even more interest to investigate. The questions we want to find answers for are 
...
There are several ways to explore what feature we need to keep around to make our prediction. The labour intensive and the - what I would like to call - blind way, but much quicker. For the purpose of gaining more inside, I choose to do both.

In the first workflow, I used several classifiers from the `sklearn.ensemble` Library. These are, `AdaBoostClassifier`,  `RandomForestClassifier`, `GradientBoostingClassifier` and `ExtraTreeClassifier`. All these classifiers have one thing in comman, and that is its attribute `feature_importances_`. Which returns the feature importance (the higher, the more important the feature).
...
The result was a pandas data frame with all features from the dataset ordered by importance, which allows us to pick the best features.  Interestingly, `RandomForestClassifier` and `ExtraTreeClassifier` had the most similar results. Other findings from that list were, 

* `Gradian Boosting` shows similar names just in a different order compared to `Random Forest` and `Extra Tree`.
* `AdaBoost` on the other hand shows an exciting and unique result. The top 8 feature alone is enough to make a good class prediction. Compare to all the other classifiers, here we have `Wilderness_Area4` on top, before `Elevation`.
...
* In regards to `Soil_Type`, it is hard to find some commonality. Here are once I choose to select: `Soil_Type2`, `Soil_Type4`, `Soil_Type10`, `Soil_Type22`, `Soil_Type23` and `Soil_Type39`.

Now, the question I asked myself at that point, is that the time wisely spend? Alternatively, worse it at all? After all, it took almost 20 minutes to calculate. One advantage that approach has is, we know our selected features.
...
The big advantage here is the short processing time it takes. On the other hand, we are losing any knowledge regarding our features. Now is that important? Maybe.
...
By selecting our features by hand so to speak, we get an `accuracy of 92.23%` and an `f1_score of 86.74%` using cross validation. In contrast, `PCA` has an `accuracy of 90.72%` and an `f1_score of 84.44%`. Now is that a big enough difference to justify the time spent to find our features? Maybe. 

To test how accurate we can predict the `Cover_Type` we used the `KNeighborsClassifier` from sklearn, and we got an accuracy of `91.02%`. That is great.


## Conclusion

Now, what can we tell about that? Yes, we can build a model that predicts what types of trees grow in an area based on the surrounding characteristics.
