...
There are several ways to explore what features we need to keep around to make our prediction. The labour intensive workflow and the much quicker workflow - what I would like to call the "blind folded method". For the purpose of gaining more inside, I choose to do both.
...
The big advantage to this is the short processing time. But the drawback, we are losing any knowledge regarding our features. Is that important? Possibly.

As you can see in the code snippet above I opted to scale the features. Researching that topic, I found that there is a multitude of approaches, where some use a combination of `StandardScaler`, `MinMaxScaler` and `Normalizer`. Scale only the continuous features and not the categorical, etcetera. For now, I just used the simplest way.
